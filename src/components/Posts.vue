    ...
    <div class="posts-management mx-auto max-sm:w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] max-lg:w-[50%] my-36">
        <div class="post-naming w-full text-center">
            <input @keyup.enter="addNewPost" type="text" v-model="title" placeholder="Name your new document"
                class="p-2 w-full rounded-lg shadow-lg bg-white dark:bg-gray-800 dark:text-white">
        </div>
        ...
        <div class="posts-section mt-14 mx-auto">
            <ul class="posts grid max-sm:grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <li class="post p-5 aspect-video rounded-lg grid justify-center items-center">
                    <div class="post-contents grid gap-y-8">
                        <div @click="addNewPost"
                            class="post-edit-btn cursor-pointer rounded-full border w-12 h-12 grid justify-center items-center border-gray-700 hover:bg-gray-700 text-gray-700 hover:text-white duration-300 dark:border-white dark:hover:bg-white dark:text-white dark:hover:text-gray-700">
                ...
                <li v-for="post of posts" :key="post?.id" :id="`post${post?.id}`"
                    class="post bg-white p-5 aspect-video rounded-lg shadow-lg grid justify-center items-center dark:bg-gray-800">
                    <div class="post-contents grid gap-y-8">

                        <div class="post-title text-xl text-center text-stone-800 dark:text-white" :id="`postTitle${post?.id}`" contenteditable="true"
